...
export function SocialGraphView({ agents, graph, active, selectedAgentId, onSelectAgent }: {agents:Agent[]; graph:Edge[]; active:number[]; selectedAgentId?:number; onSelectAgent?:(id:number)=>void}) {
  const { t } = useI18n();
  ...
  return <section className="graph-panel"><div className="panel-label"><span>{t('followGraph')}</span><small>{t('presentationOnly')}</small></div>
    <svg className="graph" viewBox="0 0 100 100" onPointerMove={reposition} onPointerUp={() => setDragged(null)} onPointerLeave={() => setDragged(null)}>
      ...
      {agents.map(agent => { const node=indexed.get(agent.id); if (!node) return null; const isActive=active.includes(agent.id); const isSelected=selectedAgentId===agent.id; return <g key={agent.id} transform={`translate(${node.x} ${node.y})`} className={`agent ${isActive ? 'active':''} ${isSelected ? 'selected':''}`} onPointerDown={event => { event.currentTarget.setPointerCapture(event.pointerId); setDragged(agent.id); }} onClick={() => onSelectAgent?.(agent.id)} role="button" tabIndex={0} aria-label={`Inspect ${agent.name}`}>
